...
class AnimeRepository(private var animeDAO: AnimeDAO,
        ...
        launch {
            try {
                ...
                if (resp.isSuccessful) {
                    ...
                    animeFullJson?.let {
                        val dataJson = it.get("data").asJsonObject
                        val genresJsonArray = dataJson.get("genres").asJsonArray
                        val studiosJsonArray = dataJson.get("studios").asJsonArray
    ...
    fun getAnimeBySearchFromServ(pageIndex: Int, text: String): LiveData<MainResponse> {
        val queryResult = MutableLiveData<MainResponse>()
        launch {
            try {
